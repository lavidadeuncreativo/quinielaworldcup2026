...
const medal = (rank:number) => rank === 1 ? "1" : rank === 2 ? "2" : rank === 3 ? "3" : String(rank);
...
export default function RankingShareCard({table,source,completed,generatedAt}:Props){
  ...
  return <section className="share-card">
    <header>
      <div className="share-brand"><b>Q</b><div><strong>Quiniela Familiar 2026</strong><span>Tabla de posiciones</span></div></div>
    ...
    <div className="share-podium">
      {top.map(player=><article key={player.id} className={`share-podium-card rank-${player.rank}`}>
        <i>{medal(player.rank)}</i>
    ...
    <div className="share-list">
      {rest.map(player=><article key={player.id}>
        <i>{player.rank}</i>
