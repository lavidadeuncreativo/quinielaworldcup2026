...
type FixtureInfo = { dateUtc:string; status:string; venue?:string };
type FixtureMap = Record<string, FixtureInfo>;
type SyncPayload = { results:ResultMap; fixtures:FixtureMap };
...
type ApiFixture = { fixture:{ id:number; date:string; status:{ short:string }; venue?:{ name?:string|null; city?:string|null } }; teams:{ home:{ name:string; winner:boolean|null }; away:{ name:string; winner:boolean|null } }; goals:{ home:number|null; away:number|null }; score:{ fulltime:{ home:number|null; away:number|null } } };
...
async function loadFromApiFootball():Promise<SyncPayload|null>{
 ...
 const response=await fetch(endpoint,{headers:{"x-apisports-key":apiKey},next:{revalidate:7200}});
 ...
 const results:ResultMap={};const fixtures:FixtureMap={};
 for(const item of items){
  ...
  fixtures[match.id]={dateUtc:item.fixture.date,status:item.fixture.status.short,venue:venueName(item)};
  if(!finishedStatuses.has(item.fixture.status.short))continue;
  const teams=matchTeams(match,results),reversed=teams.home===apiAway&&teams.away===apiHome,rawHome=item.score.fulltime.home??item.goals.home,rawAway=item.score.fulltime.away??item.goals.away;if(rawHome===null||rawAway===null)continue;
...
export async function GET(){
 try{
  ...
  return NextResponse.json({results,fixtures:apiPayload?.fixtures??{},source,refreshSeconds:7200,generatedAt:new Date().toISOString()},{headers:{"Cache-Control":"public, s-maxage=7200, stale-while-revalidate=300"}});
 }catch(error){console.error(error);return NextResponse.json({results:{},fixtures:{},source:"demo",error:"No fue posible sincronizar resultados."},{status:200})}
}
